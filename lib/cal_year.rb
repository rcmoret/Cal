...
class EntireYear < IndividualMonth
  ...
  def create_year_array
    ...
    months_iterations.each do |month|
      ...
      single_month_array[0] = this_instance.month_name_text.center(NUM_CHARS_IN_LINE)
      single_month_array[1] = this_instance.build_array_of_individual_lines
      # This line will insure that all of the months have the same number of lines
      single_month_array.flatten!
      single_month_array[7] = " " * NUM_CHARS_IN_LINE if single_month_array.size < 8
      master_array[month - 1] = single_month_array.flatten
    end
    return master_array
  end
  
  def organize_whole_year
    ...
    quarters = []
    (create_year_array.size/3).times do |i|
      quarters.push create_year_array[i*3..(i*3)+2]
    end
